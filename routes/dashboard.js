...
// Student Dashboard
const { exec } = require('child_process'); // ✅ make sure this is at the top (only once)
...
const studyHours = completedSessions;

let attendance = 0;

if (totalSessions > 0) {
  attendance = Math.round((completedSessions / totalSessions) * 100);
}

exec(`python predict.py ${studyHours} ${attendance}`, (error, stdout) => {
  ...
  let result = parseFloat(stdout);

  // ✅ FIX: Prevent invalid values
...
if (totalSessions === 0) {
  noShowRisk = "No data yet";
} else if (attendance < 50) {
  noShowRisk = "High risk ❌";
} else if (attendance < 80) {
  noShowRisk = "Moderate risk ⚠️";
} else {
  noShowRisk = "Low risk ✅";
}
...
}); // ✅ closes student dashboard route ✅
// Tutor Dashboard
