...
// ML Prediction Route
router.get('/predict/:value', (req, res) => {
    const value = req.params.value;

    exec(`python predict.py ${value}`, (error, stdout, stderr) => {
        if (error) {
            console.error("ERROR:", error);
            console.error("STDERR:", stderr);
            return res.status(500).send("ML Error: " + stderr);
        }
...
router.get('/predict-performance/:hours/:attendance', (req, res) => {
    const { hours, attendance } = req.params;

    exec(`python predict.py ${hours} ${attendance}`, (error, stdout, stderr) => {
        if (error) {
            console.error("ERROR:", error);
            console.error("STDERR:", stderr);
            return res.status(500).send("ML Error: " + stderr);
